...
//!
//! The backends are deliberately kept behind one enum rather than a trait:
...
//! loop runner — is backend-agnostic, because all CLIs are normalized onto the
...
/// `agentBackend: "claude" | "codex" | "opencode"`.
...
impl Backend {
    ...
    /// for it and the iteration cap is the effective guard. OpenCode's
    /// `step_finish` carries a real `cost`, so its spend is live. Callers surface
    /// this rather than silently pretending a $0 spend.
    pub fn reports_cost(self) -> bool {
        matches!(self, Backend::Claude | Backend::Opencode)
    }
}
...
mod tests {
    ...
    fn serializes_lowercase_and_defaults_to_claude() {
        ...
        assert_eq!(
            serde_json::to_string(&Backend::Opencode).unwrap(),
            "\"opencode\""
        );
        let b: Backend = serde_json::from_str("\"claude\"").unwrap();
        assert_eq!(b, Backend::Claude);
        let b: Backend = serde_json::from_str("\"opencode\"").unwrap();
        assert_eq!(b, Backend::Opencode);
    }

    #[test]
    fn claude_and_opencode_report_cost_but_codex_does_not() {
        assert!(Backend::Claude.reports_cost());
        assert!(Backend::Opencode.reports_cost());
