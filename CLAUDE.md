...
- After C# code changes, call `unity_wait_for_compile` then `unity_get_console_logs` (level=warning_or_error).
...
- No write tools yet. Snapshots + action log primitives exist (`packages/safety`); write tools land after they are wired through.
