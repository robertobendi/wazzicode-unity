...
| `Editor/MainThreadDispatcher.cs` | Drains queued actions on `EditorApplication.update` |
| `Editor/BridgeServer.cs` | `HttpListener` on `127.0.0.1:38578`, JSON-RPC dispatch |
...
3. Listener thread blocks on `ManualResetEventSlim` (15s timeout).
...
PNGs are returned as base64 strings inside the bridge envelope. The MCP server detects this and emits a multimodal `image` content block alongside the JSON envelope so Claude actually sees the frame.

## What's intentionally not here yet

- Play-mode controls.
- Runtime object inspection.
- Write methods (create GameObject, set serialized field, save scene, …). These wait on the snapshot + action-log path being live and tested.
